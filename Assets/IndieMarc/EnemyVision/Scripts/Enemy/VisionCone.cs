...
namespace IndieMarc.EnemyVision
{
    ...
    public class VisionCone : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (GetComponentInParent<MovementController>())
            {

                transform.forward = Vector3.Lerp(transform.forward, GetComponentInParent<CharacterController>().velocity, Time.deltaTime * 6f);
            }
            else
            {

                transform.forward = Vector3.Lerp(transform.forward, GetComponentInParent<NavMeshAgent>().velocity, Time.deltaTime * 6f);
            }

...
namespace IndieMarc.EnemyVision
{
    ...
    public class EnemyVision : MonoBehaviour
    {
        ...
        void Start()
        {
            if (vision_prefab)
            {
                vis = Instantiate(vision_prefab, GetEye(), Quaternion.identity);
        ...
        private void DetectVisionTargetOnly()
        {
            ...
            if (CanSeeVisionTargetNear(target))
            {
                if (CanSeeObject(target.gameObject,vision_near_range, vision_angle))
                {
                    ...
                    if (target != null && detect_timer > detect_time)
                    {
                        ...
                        if (detect)
                        {
                            ...
                            target.GetComponent<Hider>().Catch();
                        }
                        vision_timer = 0f;
                    }
                }
